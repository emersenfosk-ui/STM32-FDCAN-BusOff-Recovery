...
#ifndef FDCAN_FD_BRS
#define FDCAN_FD_BRS 0x00000002U
#endif
...
void bsp_InitCan1(void)
{
    ...
    sFilterConfig1.FilterID1 = 0x7E0;  //0x111;
    sFilterConfig1.FilterID2 = 0x7FF;  //0x7FF;
...
void can1_SendFD_Packet(uint8_t *_DataBuf, uint8_t _Len)
{
    ...
    TxHeader.FDFormat = FDCAN_FD_BRS;           /* 真正的 CAN-FD 帧格式，决定"帧的格式（是传统 CAN 还是 FDCAN） */
    TxHeader.TxEventFifoControl = FDCAN_NO_TX_EVENTS;
    TxHeader.MessageMarker = 0;
    HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan1, &TxHeader, _DataBuf);
}
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs)
{
    if (hfdcan == &hfdcan1) {
        if ((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET) {
            ...
            APP_CAN_HandleRx(hfdcan, g_Can1RxHeader.Identifier, g_Can1RxData, (g_Can1RxHeader.DataLength >> 16) & 0x0F);
	            /* ↓↓↓ 把 UDS 处理放到这里 ↓↓↓ */
